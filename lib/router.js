...
positionSymptomCheckerController = RouteController.extend({
...
/*  nextPath: function() {
    return Router.routes.mainSymptomChecker.path({position: this.params.position});
  },
*/
  data: function() {    
//    var position = this.position();
    //this.params needs to be evaluated within the context of the route, which only can happen if data is a function and not an object literal.
    var positionNumber = parseInt(this.params.position,10);
//    var nextPath = this.nextPath();
    return {
      symptoms: function() {
        return Symptoms.find({position: positionNumber}, {sort: {name: 1}});
      },
...
Router.map(function () {
  ...
  path: '/mainSymptomChecker/:position?',
  controller: 'positionSymptomCheckerController',
